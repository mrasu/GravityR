...
type tableFieldColumns map[string][]*FieldColumn
...
func buildTableFields(dbTables map[string]*TableSchema, scope *StmtScope) (tableFieldColumns, error) {
...
func (tfb *tableFieldsBuilder) calculateReferences(info *scopeInfo) ([]*outputCol, tableFieldColumns, error) {
	...
	tableMap := lo.SliceToMap(scope.Tables, func(t *Table) (string, *Table) {
		return t.Name, t
	})
...
type scopeInfo struct {
	dbTables map[string]*TableSchema
...
func newScopeInfo(dbTables map[string]*TableSchema, scope *StmtScope, externalCols tableOutputCols) *scopeInfo {
...
func buildReferencingColumns(dbTables map[string]*TableSchema, tables []*Table) tableOutputCols {
	res := tableOutputCols{}
...
func (si *scopeInfo) createChild(childScope *StmtScope, externalCols tableOutputCols) *scopeInfo {
...
func (si *scopeInfo) getColumnReferenceColumnInfos(c *FieldColumn) ([]*columnInfo, error) {
	if c.Type == FieldSubquery {
		return nil, nil
	}
...
func (si *scopeInfo) addReferencingField(tableName string, col *FieldColumn) {
	cols, ok := si.referencingFields[tableName]
	if !ok {
		si.referencingFields[tableName] = []*FieldColumn{col}
		return
	}

	for i, c := range cols {
		if c.Name == col.Name {
			if c.Type == FieldReference && col.Type == FieldCondition {
...
func newRefTables(referencingColumns tableOutputCols, tables []*Table, externalCols tableOutputCols) *refTables {
	asToName := map[string]string{}
...
func (t *refTables) getReferencingColumnInfos(c *FieldColumn) ([]*columnInfo, error) {
