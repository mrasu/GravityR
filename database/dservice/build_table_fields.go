...
type tableFieldColumns map[string][]*dmodel.FieldColumn
...
func buildTableFields(dbTables map[string]*dmodel.TableSchema, scope *dmodel.StmtScope) (tableFieldColumns, error) {
...
func (tfb *tableFieldsBuilder) calculateReferences(info *scopeInfo) ([]*outputCol, tableFieldColumns, error) {
	...
	tableMap := lo.SliceToMap(scope.Tables, func(t *dmodel.Table) (string, *dmodel.Table) {
		return t.Name, t
	})
...
type scopeInfo struct {
	dbTables map[string]*dmodel.TableSchema
...
func newScopeInfo(dbTables map[string]*dmodel.TableSchema, scope *dmodel.StmtScope, externalCols tableOutputCols) *scopeInfo {
...
func buildReferencingColumns(dbTables map[string]*dmodel.TableSchema, tables []*dmodel.Table) tableOutputCols {
	res := tableOutputCols{}
...
func (si *scopeInfo) createChild(childScope *dmodel.StmtScope, externalCols tableOutputCols) *scopeInfo {
...
func (si *scopeInfo) getColumnReferenceColumnInfos(c *dmodel.FieldColumn) ([]*columnInfo, error) {
	if c.Type == dmodel.FieldSubquery {
		return nil, nil
	}
...
func (si *scopeInfo) addReferencingField(tableName string, col *dmodel.FieldColumn) {
	cols, ok := si.referencingFields[tableName]
	if !ok {
		si.referencingFields[tableName] = []*dmodel.FieldColumn{col}
		return
	}

	for i, c := range cols {
		if c.Name == col.Name {
			if c.Type == dmodel.FieldReference && col.Type == dmodel.FieldCondition {
...
func newRefTables(referencingColumns tableOutputCols, tables []*dmodel.Table, externalCols tableOutputCols) *refTables {
	asToName := map[string]string{}
...
func (t *refTables) getReferencingColumnInfos(c *dmodel.FieldColumn) ([]*columnInfo, error) {
