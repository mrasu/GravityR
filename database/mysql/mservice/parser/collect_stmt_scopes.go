...
type scopeCollector struct {
	...
	scopeStack *lib.Stack[dparser.StmtScope]
	lastError  error
}

func CollectStmtScopes(rootNode ast.StmtNode) ([]*dparser.StmtScope, []error) {
	sc := &scopeCollector{scopeStack: lib.NewStack[dparser.StmtScope]()}
...
func (sc *scopeCollector) Enter(in ast.Node) (ast.Node, bool) {
	...
	scope := &dparser.StmtScope{Name: dparser.RootScopeName}
	var foundFields []*dparser.Field
	for _, field := range stmt.Fields.Fields {
		var cols []*dparser.FieldColumn
		if field.Expr != nil {
			cols = sc.collectExprReferences(field.Expr, dparser.FieldReference)
			if err := sc.getAndClearLastError(); err != nil {
				sc.errors = append(sc.errors, err)
				continue
			}
		} else {
			cols = []*dparser.FieldColumn{{
				Table: field.WildCard.Table.L,
				Type:  dparser.FieldStar,
			}}
		}
	...
	if stmt.Where != nil {
		f, err := sc.createFieldFromExpr(stmt.Where, dparser.FieldCondition)
		if err == nil {
	...
	if stmt.GroupBy != nil {
		for _, item := range stmt.GroupBy.Items {
			f, err := sc.createFieldFromExpr(item.Expr, dparser.FieldReference)
			if err == nil {
				foundFields = append(foundFields, f)
			} else {
				sc.errors = append(sc.errors, err)
	...
	if stmt.Limit != nil {
		f, err := sc.createFieldFromExpr(stmt.Limit.Count, dparser.FieldReference)
		if err == nil {
			foundFields = append(foundFields, f)
		} else {
			sc.errors = append(sc.errors, err)
		...
		f, err = sc.createFieldFromExpr(stmt.Limit.Offset, dparser.FieldReference)
		if err == nil {
			foundFields = append(foundFields, f)
		} else {
			sc.errors = append(sc.errors, err)
...
func (sc *scopeCollector) createFieldFromExpr(expr ast.ExprNode, fieldType dparser.FieldType) (*dparser.Field, error) {
...
func (sc *scopeCollector) collectExprReferences(expr ast.ExprNode, defaultType dparser.FieldType) []*dparser.FieldColumn {
	if expr == nil {
		return nil
	}
	...
	case *ast.SubqueryExpr:
		...
		return []*dparser.FieldColumn{{Type: dparser.FieldSubquery}}
	...
	case *ast.ColumnNameExpr:
		return []*dparser.FieldColumn{{Table: e.Name.Table.L, Name: e.Name.Name.L, Type: defaultType}}
	...
	case *ast.MatchAgainst:
		...
		for _, c := range e.ColumnNames {
			res = append(res, &dparser.FieldColumn{Table: c.Table.L, Name: c.Name.L, Type: defaultType})
		}
...
func (sc *scopeCollector) collectTables(ref *ast.TableRefsClause) ([]*dparser.Table, []*dparser.Field) {
	if ref == nil {
		return nil, nil
	}
...
func (sc *scopeCollector) collectReferencingTables(resultSet ast.ResultSetNode) ([]*dparser.Table, []*dparser.Field) {
	if resultSet == nil {
		return nil, nil
	}
	...
	case *ast.TableSource:
		...
		case *ast.TableName:
			...
			tables = append(tables, &dparser.Table{AsName: n.AsName.L, Name: src.Name.L})
		case *ast.SelectStmt:
			tables = append(tables, &dparser.Table{AsName: n.AsName.L})
		case *ast.SetOprStmt:
...
func (sc *scopeCollector) collectJoinReferences(j *ast.Join) ([]*dparser.Table, []*dparser.Field) {
	if j == nil {
		return nil, nil
	}
	...
	if j.On != nil {
		fields = append(fields, &dparser.Field{Columns: sc.collectExprReferences(j.On.Expr, dparser.FieldCondition)})
	}
