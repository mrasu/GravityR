...
type scopeCollector struct {
	...
	scopeStack *lib.Stack[dmodel.StmtScope]
	lastError  error
}

func CollectStmtScopes(rootNode ast.StmtNode) ([]*dmodel.StmtScope, []error) {
	sc := &scopeCollector{scopeStack: lib.NewStack[dmodel.StmtScope]()}
...
func (sc *scopeCollector) Enter(in ast.Node) (ast.Node, bool) {
	...
	scope := &dmodel.StmtScope{Name: dmodel.RootScopeName}
	var foundFields []*dmodel.Field
	for _, field := range stmt.Fields.Fields {
		var cols []*dmodel.FieldColumn
		if field.Expr != nil {
			cols = sc.collectExprReferences(field.Expr, dmodel.FieldReference)
			if err := sc.getAndClearLastError(); err != nil {
				sc.errors = append(sc.errors, err)
				continue
			}
		} else {
			cols = []*dmodel.FieldColumn{{
				Table: field.WildCard.Table.L,
				Type:  dmodel.FieldStar,
			}}
		}
	...
	if stmt.Where != nil {
		f, err := sc.createFieldFromExpr(stmt.Where, dmodel.FieldCondition)
		if err == nil {
	...
	if stmt.GroupBy != nil {
		for _, item := range stmt.GroupBy.Items {
			f, err := sc.createFieldFromExpr(item.Expr, dmodel.FieldReference)
			if err == nil {
				foundFields = append(foundFields, f)
			} else {
				sc.errors = append(sc.errors, err)
	...
	if stmt.Limit != nil {
		f, err := sc.createFieldFromExpr(stmt.Limit.Count, dmodel.FieldReference)
		if err == nil {
			foundFields = append(foundFields, f)
		} else {
			sc.errors = append(sc.errors, err)
		...
		f, err = sc.createFieldFromExpr(stmt.Limit.Offset, dmodel.FieldReference)
		if err == nil {
			foundFields = append(foundFields, f)
		} else {
			sc.errors = append(sc.errors, err)
...
func (sc *scopeCollector) createFieldFromExpr(expr ast.ExprNode, fieldType dmodel.FieldType) (*dmodel.Field, error) {
...
func (sc *scopeCollector) collectExprReferences(expr ast.ExprNode, defaultType dmodel.FieldType) []*dmodel.FieldColumn {
	if expr == nil {
		return nil
	}
	...
	case *ast.SubqueryExpr:
		...
		return []*dmodel.FieldColumn{{Type: dmodel.FieldSubquery}}
	...
	case *ast.ColumnNameExpr:
		return []*dmodel.FieldColumn{{Table: e.Name.Table.L, Name: e.Name.Name.L, Type: defaultType}}
	...
	case *ast.MatchAgainst:
		...
		for _, c := range e.ColumnNames {
			res = append(res, &dmodel.FieldColumn{Table: c.Table.L, Name: c.Name.L, Type: defaultType})
		}
...
func (sc *scopeCollector) collectTables(ref *ast.TableRefsClause) ([]*dmodel.Table, []*dmodel.Field) {
	if ref == nil {
		return nil, nil
	}
...
func (sc *scopeCollector) collectReferencingTables(resultSet ast.ResultSetNode) ([]*dmodel.Table, []*dmodel.Field) {
	if resultSet == nil {
		return nil, nil
	}
	...
	case *ast.TableSource:
		...
		case *ast.TableName:
			...
			tables = append(tables, &dmodel.Table{AsName: n.AsName.L, Name: src.Name.L})
		case *ast.SelectStmt:
			tables = append(tables, &dmodel.Table{AsName: n.AsName.L})
		case *ast.SetOprStmt:
...
func (sc *scopeCollector) collectJoinReferences(j *ast.Join) ([]*dmodel.Table, []*dmodel.Field) {
	if j == nil {
		return nil, nil
	}
	...
	if j.On != nil {
		fields = append(fields, &dmodel.Field{Columns: sc.collectExprReferences(j.On.Expr, dmodel.FieldCondition)})
	}
