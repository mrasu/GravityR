...
var tableColumns = map[string][]*dmodel.ColumnSchema{
	"users": {
		{Name: "id"},
		{Name: "name"},
		{Name: "email"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			thelper.MockMysqlDB(t, func(db *mysql.DB, mock sqlmock.Sqlmock) {
				...
				ts, err := mservice.CollectTableSchemas(db, thelper.DBName, tt.tables)
...
func TestCollectTableSchemas_Error(t *testing.T) {
	thelper.MockMysqlDB(t, func(db *mysql.DB, mock sqlmock.Sqlmock) {
		...
		ts, err := mservice.CollectTableSchemas(db, thelper.DBName, []string{"missing"})
		assert.Nil(t, ts)
		assert.Error(t, err)
	})
}
