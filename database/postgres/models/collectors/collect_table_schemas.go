...
func CollectTableSchemas(db *postgres.DB, schema string, tables []string) ([]*db_models.TableSchema, error) {
	cols, err := db.GetTableColumns(schema, tables)
	if err != nil {
		return nil, err
	}

	schemas := db_models.CreateTableSchemas(tables, cols, func(c *postgres.ColumnInfo) (string, string, bool) {
