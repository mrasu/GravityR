...
func CollectStmtScopes(stmt *parser.Statement, targetSchema string) ([]*dmodel.StmtScope, []error) {
...
func (sc *scopeCollector) Enter(node interface{}) bool {
	...
	var foundFields []*dmodel.Field
	for _, expr := range stmt.Exprs {
		cols := sc.collectExprReferences(expr.Expr, dmodel.FieldReference)
		if err := sc.getAndClearLastError(); err != nil {
			sc.errors = append(sc.errors, err)
			continue
		}

		foundFields = append(foundFields, &dmodel.Field{
	...
	if stmt.Where != nil {
		f, err := sc.createFieldFromExpr(stmt.Where.Expr, dmodel.FieldCondition)
		if err == nil {
	...
	if stmt.GroupBy != nil {
		for _, g := range stmt.GroupBy {
			f, err := sc.createFieldFromExpr(g, dmodel.FieldReference)
			if err == nil {
				foundFields = append(foundFields, f)
			} else {
				sc.errors = append(sc.errors, err)
	...
	if stmt.Having != nil {
		f, err := sc.createFieldFromExpr(stmt.Having.Expr, dmodel.FieldReference)
		if err == nil {
			foundFields = append(foundFields, f)
		} else {
			sc.errors = append(sc.errors, err)
	...
	if selectNode.OrderBy != nil {
		for _, o := range selectNode.OrderBy {
			f, err := sc.createFieldFromExpr(o.Expr, dmodel.FieldReference)
			if err == nil {
				foundFields = append(foundFields, f)
			} else {
				sc.errors = append(sc.errors, err)
	...
	if selectNode.Limit != nil {
		f, err := sc.createFieldFromExpr(selectNode.Limit.Count, dmodel.FieldReference)
		if err == nil {
			foundFields = append(foundFields, f)
		} else {
			sc.errors = append(sc.errors, err)
		...
		f, err = sc.createFieldFromExpr(selectNode.Limit.Offset, dmodel.FieldReference)
		if err == nil {
			foundFields = append(foundFields, f)
		} else {
			sc.errors = append(sc.errors, err)
...
func (sc *scopeCollector) newStmtScopeIfEnterRequired(node interface{}) (*dmodel.StmtScope, *tree.Select) {
	...
	if sub, ok := node.(*tree.Subquery); ok {
		...
		scope := &dmodel.StmtScope{Name: sInfo.name}
...
func (sc *scopeCollector) createFieldFromExpr(expr tree.Expr, fieldType dmodel.FieldType) (*dmodel.Field, error) {
	cols := sc.collectExprReferences(expr, fieldType)
	if err := sc.getAndClearLastError(); err != nil {
		return nil, err
	}

	return &dmodel.Field{Columns: cols}, nil
}

func (sc *scopeCollector) collectExprReferences(expr tree.Expr, defaultType dmodel.FieldType) []*dmodel.FieldColumn {
	if expr == nil {
		return nil
	}

	var res []*dmodel.FieldColumn
	...
	case *tree.Subquery:
		...
		return []*dmodel.FieldColumn{{
			Type:          dmodel.FieldSubquery,
	...
	case *tree.TupleStar:
		if n, ok := e.Expr.(*tree.UnresolvedName); ok {
			if sc.isSchemaSpecified(n, 1) {
				...
				return []*dmodel.FieldColumn{{Table: n.Parts[0], Type: dmodel.FieldStar}}
			}
		} else {
			res = append(res, sc.collectExprReferences(e.Expr, defaultType)...)
		}
	case *tree.ColumnAccessExpr:
		if n, ok := e.Expr.(*tree.UnresolvedName); ok {
			if sc.isSchemaSpecified(n, 1) {
				...
				return []*dmodel.FieldColumn{{Table: n.Parts[0], Name: e.ColName, Type: defaultType}}
			}
		} else {
			res = append(res, sc.collectExprReferences(e.Expr, defaultType)...)
		}
	...
	case *tree.UnresolvedName:
		if sc.isSchemaSpecified(e, 2) {
			...
			if e.Star {
				return []*dmodel.FieldColumn{{Table: e.Parts[1], Name: "", Type: dmodel.FieldStar}}
			} else {
				return []*dmodel.FieldColumn{{Table: e.Parts[1], Name: e.Parts[0], Type: defaultType}}
			}
		}
	case tree.UnqualifiedStar:
		return []*dmodel.FieldColumn{{Table: "", Type: dmodel.FieldStar}}
	case *tree.AllColumnsSelector:
		if sc.isSchemaSpecified2(e.TableName, 1) {
			sc.lastError = lib.NewUnsupportedError("schema or catalog/db specification is not supported")
		} else {
			return []*dmodel.FieldColumn{{Table: e.TableName.Parts[0], Type: dmodel.FieldStar}}
		}
	case *tree.ColumnItem:
		if sc.isSchemaSpecified2(e.TableName, 1) {
			sc.lastError = lib.NewUnsupportedError("schema or catalog/db specification is not supported")
		} else {
			return []*dmodel.FieldColumn{{Table: e.TableName.Parts[0], Name: e.ColumnName.Normalize()}}
		}
	default:
...
func (sc *scopeCollector) collectTables(table tree.TableExpr) ([]*dmodel.Table, []*dmodel.Field) {
	if table == nil {
		return nil, nil
	}

	var tables []*dmodel.Table
	var fields []*dmodel.Field
	switch t := table.(type) {
	case *tree.AliasedTableExpr:
		...
		case *tree.UnresolvedObjectName:
			if sc.isSchemaSpecified2(tt, 1) {
				...
				tables = []*dmodel.Table{{AsName: t.As.Alias.Normalize(), Name: tt.Parts[0]}}
			}
		...
		case *tree.RowsFromExpr:
			for _, item := range tt.Items {
				fcs := sc.collectExprReferences(item, dmodel.FieldReference)
				fields = append(fields, &dmodel.Field{Columns: fcs})
			}
		case *tree.TableName:
			if sc.isSchemaSpecifiedTableName(tt) {
				sc.lastError = lib.NewUnsupportedError("schema or catalog/db specification is not supported")
			} else {
				tables = []*dmodel.Table{{AsName: t.As.Alias.Normalize(), Name: tt.Table()}}
			}
		case *tree.TableRef:
...
func (sc *scopeCollector) collectJoinReferences(j *tree.JoinTableExpr) ([]*dmodel.Table, []*dmodel.Field) {
	if j == nil {
		return nil, nil
	}
	...
	if j.Cond != nil {
		...
		case *tree.OnJoinCond:
			fields = append(fields, &dmodel.Field{Columns: sc.collectExprReferences(c.Expr, dmodel.FieldCondition)})
		case *tree.UsingJoinCond:
			// todo: not supporting the same column name?
			for _, col := range c.Cols {
				fields = append(fields, &dmodel.Field{Columns: []*dmodel.FieldColumn{{Table: "", Name: col.Normalize(), Type: dmodel.FieldCondition}}})
			}
		default:
			sc.lastError = lib.NewUnsupportedError(
...
func (sc *scopeCollector) newScope(parent *scopeInfo, s *tree.Select) *dmodel.StmtScope {
	name := dmodel.RootScopeName
	if n, ok := sc.foundSelectInfos[s]; ok {
		name = n.name
	}
	scope := &dmodel.StmtScope{Name: name}
