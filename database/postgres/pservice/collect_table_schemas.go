...
func CollectTableSchemas(db *postgres.DB, schema string, tables []string) ([]*dmodel.TableSchema, error) {
	cols, err := db.GetTableColumns(schema, tables)
	if err != nil {
		return nil, err
	}

	schemas := dmodel.CreateTableSchemas(tables, cols, func(c *postgres.ColumnInfo) (string, string, bool) {
