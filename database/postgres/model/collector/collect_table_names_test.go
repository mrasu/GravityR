...
func TestCollectTableNames(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "union query",
			query:  "SELECT * FROM users UNION ALL SELECT * FROM todos",
			tables: []string{"users", "todos"},
		},
		{
			name:   "union query",
			query:  "SELECT * FROM users INNER JOIN (SELECT * FROM users UNION ALL SELECT * FROM todos) AS u2 ON u2.id = users.id;",
			tables: []string{"users", "todos"},
		},
		{
			name: "CTE name duplicated query",
...
order by status desc`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			actualTables, errs := collector.CollectTableNames(&stmtNodes[0])
			assert.Empty(t, errs)
