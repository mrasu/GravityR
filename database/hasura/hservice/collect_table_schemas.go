...
func CollectTableSchemas(cli *hasura.Client, schema string, tables []string) ([]*dmodel.TableSchema, error) {
	cols, err := cli.GetTableColumns(schema, tables)
	if err != nil {
		return nil, err
	}

	schemas := dmodel.CreateTableSchemas(tables, cols, func(c *hasura.ColumnInfo) (string, string, bool) {
