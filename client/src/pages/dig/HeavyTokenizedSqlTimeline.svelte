<script lang="ts">
  ...
  let chart: DbTokenizedLoadChart;
  let sqlTimelineChartDiv: HTMLElement;
  let selectedProp: DbTokenizedLoadChartProp;
  ...
  onMount(() => {
    ...
    chart = new DbTokenizedLoadChart(
      sqlTimelineChartDiv,
  ...
  const createTooltip = (prop: DbTokenizedLoadChartProp): string => {
    return createLoadMaxTooltip(prop, 300);
  };

  const onBarClick = (prop: DbTokenizedLoadChartProp) => {
    selectedProp = prop;
